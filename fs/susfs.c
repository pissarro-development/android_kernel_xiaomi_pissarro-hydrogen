...
#define SUSFS_LOGI(fmt, ...) do { if (static_branch_likely(&susfs_is_log_enabled)) pr_info("susfs:[%u][%d][%s] " fmt, current_uid().val, current->pid, __func__, ##__VA_ARGS__); } while (0)
#define SUSFS_LOGE(fmt, ...) do { if (static_branch_likely(&susfs_is_log_enabled)) pr_err("susfs:[%u][%d][%s]" fmt, current_uid().val, current->pid, __func__, ##__VA_ARGS__); } while (0)
#else
...
int susfs_open_redirect_spoof_vfs_readlink(struct inode *inode, char __user *buffer, int buflen) {
	...
	hash_for_each_possible_rcu(OPEN_REDIRECT_HLIST, entry, node, inode->i_ino) {
			...
			entry->target_dev == inode->i_sb->s_dev)
		{
			...
			srcu_read_unlock(&susfs_srcu_open_redirect, srcu_idx);
			/* readlink(2) must report the number of bytes written; returning
			 * 0 (as upstream does) makes the link appear empty instead of
			 * spoofed. The caller returns any value >= 0 verbatim. */
...
/* code is straightly borrowed from KernelSU's pkg_observer.c, adapted to the
 * 4.14 fsnotify handle_event ABI (v2.2.0 uses the 5.x handle_inode_event op).
 * We watch the underlying f2fs /data/media/0 for its child dir "Android" being
 * opened, which signals that /sdcard (FUSE) is decrypted; then we flip
 * susfs_is_sdcard_android_data_not_decrypted off (deferred, outside SRCU).
 */
...
static struct watch_dir g_watch = { .path = "/data/media/0", // we choose the underlying f2fs /data/media/0 instead of the FUSE /sdcard
									.mask = (FS_EVENT_ON_CHILD | FS_ISDIR | FS_OPEN) };
								...
								struct fsnotify_mark **out)
{
	...
	*out = m;
	/* Drop the reference taken by fsnotify_init_mark(); the group now holds
	 * its own reference and will free the mark on fsnotify_destroy_group().
	 * Without this the mark (and its group ref) leak forever. */
	fsnotify_put_mark(m);
	return 0;
}
...
static int susfs_sdcard_monitor_fn(void *data)
{
	...
	if (IS_ERR(g)) {
		ret = PTR_ERR(g);
		g = NULL; /* never leave an ERR_PTR in the global */
		return ret;
	}

	ret = watch_one_dir(&g_watch);
	if (ret) {
		/* Could not install the watch: tear the group back down so we don't
		 * leave a live group with no marks around. sus-mount faking stays
		 * always-on (key left TRUE), which is the safe/hiding behaviour. */
		fsnotify_destroy_group(xchg(&g, NULL));
	}
