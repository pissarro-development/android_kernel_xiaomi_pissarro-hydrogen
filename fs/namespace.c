...
#define CL_COPY_MNT_NS 0x100 /* used by copy_mnt_ns() */
...
static struct mount *susfs_alloc_unshare_ksu_vfsmnt(const char *name, int old_mnt_id)
{
	...
	if (mnt) {
		...
		/* mnt_id is reused from the original (not ida-allocated), so flag it
		 * now — before any 'goto out_free' path can reach mnt_free_id() and
		 * wrongly ida_free() the still-live original's id. clone_mnt() later
		 * overwrites mnt_flags from old and re-sets this bit for the success
		 * path; the window in between makes no mnt_free_id() call. */
...
struct vfsmount *susfs_get_non_sus_vfsmnt_from_vfsmnt(struct vfsmount *vfsmnt) {
	...
	if (!mnt->mnt.mnt_root || IS_ERR(mnt->mnt.mnt_root)) {
		mntput(&mnt->mnt);
		/* Fall back to the original vfsmnt. The caller balances refs by
		 * dput(->mnt_root)+mntput() on the returned mount whenever it
		 * equals path->mnt, so take those refs here to keep it balanced. */
		mntget(vfsmnt);
